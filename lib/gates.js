...
function or(c, in1, in2, out1) {
  var a = interface.matchWireSize(c, in1);
  var b = interface.matchWireSize(c, in1);
...
function xor(c, in1, in2, out1) {
  var a = interface.matchWireSize(c, in1);
  var b = interface.matchWireSize(c, in1);
  var w = interface.matchWireSize(c, in1);
...
function fulladder(c, in1, in2, in3, carry, sum) {
  var w1 = interface.matchWireSize(c, in1);
  var w2 = interface.matchWireSize(c, in1);
  var w3 = interface.matchWireSize(c, in1);
...
function inc(c, in1, carry, sum) {
  var in2 = interface.matchWireSize(c, in1);
  c.on(in2[0]);
...
function srflipflop(c, s, r, q, qp) {
  var a = interface.matchWireSize(c, s);
  var b = interface.matchWireSize(c, s);
  var clock = interface.matchClockSize(c, s);
...
function dflipflop(c, d, q, qp) {
  var s = interface.matchWireSize(c, d);
  not(c, d, s);
...
function mux(c, ins, sel, out1) {
  ...
  var orIn = interface.matchWireSize(c, out1);
  var orOut = interface.matchWireSize(c, out1);

  for(var x = 0; x < ins.length; x++) {
    var bits = interface.numToReversedPaddedBitsArray(x, sel.length);
    var andIn = ins[x];
    var andOut = interface.matchWireSize(c, out1);

    for(var i = bits.length - 1; i >= 0; i--) {
      var selbundle = interface.fanWireToMatchSize(sel[i], out1);
      var nsel = interface.matchWireSize(c, out1);
      ...
      andIn = andOut;
      andOut = interface.matchWireSize(c, out1);
    }
    ...
    if(x === ins.length - 2) {
      ...
      orOut = interface.matchWireSize(c, out1);
    }
  }
}
