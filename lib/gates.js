...
function mux(c, in1, sel, out1) {
  ...
  var temporin = c.wire();
  var temporout = c.wire();
  for(var x = 0; x < in1.length; x++) {
    var bits = ('000000' + x.toString(2)).slice(sel.length * -1).split('').reverse();
    var tempin = in1[x];
    var tempout = c.wire();
    for(var i = bits.length - 1; i >= 0; i--) {
      ...
      not(c, sel[i], nsel);
      if(bits[i] === '1') {
        and(c, tempin, sel[i], tempout);
      } else {
        and(c, tempin, nsel, tempout);
      }
      tempin = tempout;
      tempout = c.wire();
    }
    or(c, temporin, tempin, temporout);
    temporin = temporout;
    if(x === in1.length - 2) {
      temporout = out1;
    } else {
